...
// TestGetCapabilities tests that GetCapabilities returns correct backend capabilities
func TestGetCapabilities(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "dolt backend is single-process",
			cfg:            &Config{Backend: BackendDolt},
			wantSingleProc: true,
		},
		{
			name:           "empty backend defaults to sqlite (multi-process)",
			cfg:            &Config{},
			wantSingleProc: false,
		},
	}
