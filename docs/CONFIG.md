...
| `federation.remote` | - | `BD_FEDERATION_REMOTE` | (none) | Optional remote metadata (legacy) |
| `federation.sovereignty` | - | `BD_FEDERATION_SOVEREIGNTY` | (none) | Optional sovereignty metadata (legacy) |
...
**Backend note:**
- This fork supports SQLite + JSONL workflows.
...
The sync mode controls how beads synchronizes data with git/JSONL workflows.
...
#### Federation Configuration (Legacy Metadata)

These keys can still exist in config for compatibility:
